...
var caching = function(args) {
    ...
    self.tts = args.tts || false;
    // console.log('caching self.tts :>> ', self.tts);
    // console.log('caching self.refreshThreshold :>> ', self.refreshThreshold);
    ...
    function wrapPromise(key, promise, options) {
        return new Promise(function(resolve, reject) {
            var wrapSpan = apm.startSpan('wrapCache: ' + key);
    ...
    function handleBackgroundRefresh(key, work, options) {
        // console.log('call to hbr with self.refreshThreshold :>> ', self.refreshThreshold);
        if (self.refreshThreshold && !backgroundQueue.has(key)) {
            // console.log('hbr: passing to crt -> options.retrievedTTL :>> ', options.retrievedTTL);
            var bgRefreshSpan = apm.startSpan('backgroundRefresh: ' + key);
            backgroundQueue.add(key);
            self.checkRefreshThreshold(key, options.retrievedTTL, function(err, isExpiring) {
                // console.log('crt cb isExpiring :>> ', isExpiring, key);
                if (err) {
                    backgroundQueue.delete(key);
                    bgRefreshSpan && bgRefreshSpan.end();
                    return;
                }
                if (isExpiring) {
                    // console.log('ATTEMPT BGR fetching bg as it was expiring :>> ', key);
                    work(function(err, data) {
                        if (err || !self._isCacheableValue(data)) {
                            // console.log('caught an erro on bgr - exit');
                            backgroundQueue.delete(key);
                            bgRefreshSpan && bgRefreshSpan.end();
                            return;
                        }
                        ...
                        // console.log('calling set for exiring bgr :>> ');
                    ...
                    // console.log('NO ATTEMPT for BGR fetching bg as it was NOT expiring :>> ', key);
                    backgroundQueue.delete(key);
                    bgRefreshSpan && bgRefreshSpan.end();
                }
            });
     ...
     * @param {function} cb
     */
    self.checkRefreshThreshold = function(key, retrievedTTL = null, cb) {
        // console.log('called crt :>> ');
        if (self.refreshThreshold && typeof self.store.ttl === 'function') {
            //we already got the ttl
            if (retrievedTTL) {
                // console.log('crt: passed retrievedTTL :>> ', retrievedTTL);
                if (self.refreshThreshold > retrievedTTL) {
                    // console.log('crt: self.refreshThreshold > retrievedTTL :>> ', self.refreshThreshold, retrievedTTL);
                    return cb(null, true);
                }
                // console.log('self.refreshThreshold <= retrievedTTL :>> ', self.refreshThreshold, retrievedTTL);
                return cb(null, false);
            } else {
                // console.log('crt :>> normal process - call to ttl');
                // console.log('call to store.ttl in crt');
                return self.store.ttl(key, function(ttlErr, ttl) {
                    ...
                    if (self.refreshThreshold > ttl) {
                        // console.log('crt: :>> refresh callback true');
                        return cb(null, true);
                    }
                    // console.log('crt: :>> refresh callback false');
                    return cb(null, false);
                });
            }
        } else {
    ...
    self.wrap = function() {
        ...
        if (hasKey) { return; }
        if (self.tts && self.refreshThreshold && typeof self.store.ttl === 'function') {
            // console.log('wrap - have a tts :>> ', self.tts);
            // console.log('wrap - have a refreshThreshold :>> ', self.refreshThreshold);
            // console.log('call store.TTL in wrap');
            return self.store.ttl(key, function(ttlErr, ttl) {
                if ((ttlErr || typeof ttl !== 'number') && !self.ignoreCacheErrors) {
                    callbackFiller.fill(key, new Error('Invalid TTL response'));
                } else if (ttl > 0) {
                    // console.log('wrap ttl callback passed a  ttl of  :>> ', ttl, key);
                    options.retrievedTTL = ttl;
                    if (self.tts < ttl) { // only call background refresh if it is not stale tts = 23 hour ttl = 22.59
                        // console.log('wrap - we are NOT stale TTS < ttl normal get flow :>> ', self.tts, ttl, key);
                        // go get it from cache
                        // console.log('calling to get it from the cache :>> ');
                        self.store.get(key, options, function(err, result) {
                            if (err && (!self.ignoreCacheErrors)) {
                                callbackFiller.fill(key, err);
                            } else if (self._isCacheableValue(result)) {
                                // found a cacheable item in the cache
                                handleBackgroundRefresh(key, work, options);
                                // console.log('hbr - filling from cached value post check for bg refresh :>> ', key);
                        ...
                        //we are stale try to get it first, if that fails try to respond with cached value.
                        // console.log('wrap - stale -  ', self.tts, ttl, key);
                        // console.log('wrap :>> calling underlying service since it was stale', key);
                        //call work function first
                        work(function(workErr, data) {
                            if (workErr || !self._isCacheableValue(data)) {
                                // we failed to get a fresh value from the underlying service.
                                // try to get it from the cache and return results,
                                // if cache call results in an error or not cacheable return that error as the result.
                                // console.log('got an error from work func, getting from cache instead');
                                // console.log('calling to get it from the cache :>> ');
                                self.store.get(key, options, function(err, result) {
                                    if (err && (!self.ignoreCacheErrors)) {
                                        // console.log('failed to get from cache! :>> ');
                                        callbackFiller.fill(key, err);
                                        return;
                                    } else if (self._isCacheableValue(result)) {
                                        // found a cacheable item in the cache, still try another
                                        // background refresh since we failed initially
                                        // console.log('got from cache! :>> ');
                                        handleBackgroundRefresh(key, work, options);
                                        callbackFiller.fill(key, null, result);
                                        return;
                                    } else { //no luck getting it from the cache
                                        // console.log('failed to get good val backup from the cache :>> ');
                                        if (workErr) {
                                            // console.log('wrap send original service error back :>> ');
                                            callbackFiller.fill(key, workErr);
                                            return;
                                        }

                                        callbackFiller.fill(key,
                                            new Error("Unrecoverable failure attempting to refresh stale cache data"));
                                        return;
                                    }
                                });
                                ...
                                // console.log('got the stale value from underlying store', key);

                                // otherwise we got the value from the service, yippee!
                                // lets try to set the ttl and store the new data in the cache
                                if (options && typeof options.ttl === 'function') {
                                    options.ttl = options.ttl(data);
                                }
                ...
                } else { //item not in cache do the normal thing
                    // console.log('else clause for ttl method, ttl not > 0 :>> ', ttl, key);
                    work(workCb);
                }
            });
        } else { // do the normal process
            // console.log('there is either no tts or refreshThreshold set tts,refreshThreshold:', self.tts, self.refreshThreshold, key);
            // console.log('calling to get it from the cache :>> ');
            self.store.get(key, options, function(err, result) {
                if (err && (!self.ignoreCacheErrors)) {
                    callbackFiller.fill(key, err);
                } else if (self._isCacheableValue(result)) {
                    // console.log('filled with cache data:>> ');
                    handleBackgroundRefresh(key, work, options);
                    callbackFiller.fill(key, null, result);
                } else {
    ...
    // Note - wrapMultiple not currently in use, as such, support for tts was not implemented
